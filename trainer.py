...
from metrics.utils import compute_accuracy
...
class Trainer:
    def __init__(self, config, pretrained=True, augmentor=ImgAugTransformV2()):
        ...
        self.print_interval = config['trainer']['print_every']
        self.valid_interval = config['trainer']['valid_every']
        ...
        if pretrained:
            ...
            print(f"Get weights from: {weight_file}")
    ...
    def train(self, project_name = 'text-recogntion', experiment_name = None):
        ...
        wandb.define_metric("val/loss", step_metric="iteration")
        wandb.define_metric("metrics/acc_full_seq", step_metric="iteration")
        wandb.define_metric("metrics/acc_per_char", step_metric="iteration")
        wandb.define_metric("metrics/CER", step_metric="iteration") 
        wandb.define_metric("metrics/WER", step_metric="iteration")
        ...
        for _ in range(self.num_iters):
            ...
            wandb.log({"train/loss": loss, "iteration": self.iter}, step = self.iter)

            if self.iter % self.print_interval == 0:
                info = "iter: {:6d} - train loss: {:.3f} - lr: {:.2e} - load time: {:.2f} - gpu time: {:.2f}".format(
                    self.iter,
                    total_loss/self.print_interval,
            ...
            if self.iter % self.valid_interval == 0 and self.valid_annotation:
                val_loss = self.validate()
                acc_full_seq, acc_per_char, cer, wer = self.precision(self.metrics)
                info = 'iter: {0.6d} - valid loss: {:.3f} \n' \
                '- acc full seq: {:.4f} - acc per char: {:.4f} - CER: {:.4f} - WER: {:.4f}'.format(
                    self.iter, val_loss, acc_full_seq, acc_per_char, cer, wer
                )
                print(info)
                self.logger.log(info)
                wandb.log({"val/loss": val_loss, "iteration":self.iter})
                wandb.log({"metrics/acc_full_seq": acc_full_seq, "iteration": self.iter})
                wandb.log({"metrics/acc_per_char": acc_per_char, "iteration": self.iter})
                wandb.log({"metrics/CER": cer, "iteration": self.iter})
                wandb.log({"metrics/WER": wer, "iteration": self.iter})
                if wer < best_wer:
                    self.save_weights(f"{self.export_weights}--val_loss={val_loss}--wer={wer}.pth")
                    best_wer = wer
    def validate(self):
        self.model.eval()
        total_loss = []
    ...
    def predict(self, sample = None, log_iteration = False):
    ...
    def load_weights(self, filename):
        state_dict = torch.load(filename, weights_only=True, map_location=torch.device(self.device))
